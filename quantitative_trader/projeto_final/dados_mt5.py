...
def construcao_historica_cotacoes():
    ...
    for acao in acoes:
        print(f"Processando cotações para: {acao}")  # Adicione esta linha para logar qual ação está sendo processada
        try:
            cotacoes = mt5.copy_rates_range(acao, mt5.TIMEFRAME_D1, data_inicial, data_final)
            if cotacoes is None or len(cotacoes) == 0:  # Verifique se as cotações estão vazias
                print(f"Sem cotações disponíveis para: {acao}")
                continue  # Se estiver vazio, vá para a próxima iteração
            ...
            lista_df_cotacoes.append(cotacoes)  # Só adicione se as cotações não estiverem vazias
        except Exception as e:
            print(f"Erro ao processar cotações para {acao}: {e}")  # Isso imprimirá o erro se ocorrer

    if not lista_df_cotacoes:  # Verifique se a lista está vazia
        print("Não há cotações para concatenar.")
        return pd.DataFrame()  # Retorne um DataFrame vazio para evitar erros
